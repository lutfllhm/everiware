...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  Future<void> _syncOffline() async {
    ...
    final synced = await OfflineQueue().syncAll(
      onProgress: (msg) { if (mounted) _snack(msg); },
    );
    if (mounted) {
      setState(() => _syncing = false);
      _checkOfflineQueue();
      if (synced > 0) {
        _snack('$synced absensi berhasil disinkronkan  - ');
      } else {
        _snack('Tidak ada data yang perlu disinkronkan');
      }
    }
  }
              ...
              sliver: SliverList(
                delegate: SliverChildListDelegate([
                  ...
                  _SettingsSection(
                    ...
                    children: [
                      _buildStatsRow(isLast: false),
                      _buildHelpdeskRow(isLast: true),
                    ],
                  ),
                  const SizedBox(height: 12),
                  _SettingsSection(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                children: [
                  ...
                  GestureDetector(
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(
                        color: _editMode ? AppColors.dangerBg : AppColors.primary.withValues(alpha: 0.06),
                        borderRadius: BorderRadius.circular(20),
                        border: Border.all(
                          color: _editMode ? AppColors.danger.withValues(alpha: 0.2) : AppColors.primary.withValues(alpha: 0.15),
                        ),
                      ),
                      child: Text(
            ...
            if (_editMode) ...[
              ...
              _NewInfoRow(
                ...
                value: (user?.phone == null || user!.phone!.isEmpty) ? 'Belum diisi' : user.phone!,
                isEmpty: user?.phone == null || (user?.phone?.isEmpty ?? true),
              ),
              _NewInfoRow(
  ...
  Widget _buildHelpdeskRow({bool isLast = false}) => _SettingsRow(
        icon: Icons.help_outline_rounded,
        iconColor: AppColors.primary,
        iconBgColor: AppColors.primary.withValues(alpha: 0.06),
        title: 'Pusat Bantuan',
        subtitle: 'Hubungi customer service & bantuan',
        isLast: isLast,
        onTap: () {
          HapticFeedback.lightImpact();
          Navigator.push(
  ...
  Widget _buildStatsRow({bool isLast = false}) => _SettingsRow(
        icon: Icons.bar_chart_rounded,
        iconColor: AppColors.primary,
        iconBgColor: AppColors.primary.withValues(alpha: 0.06),
        title: 'Statistik User',
        subtitle: 'Lihat diagram & aktivitas pengguna',
        isLast: isLast,
        onTap: () {
          HapticFeedback.lightImpact();
          Navigator.push(
  ...
  Widget _buildOfflineSyncCard() => Container(
        decoration: BoxDecoration(
          color: Colors.white,
          borderRadius: BorderRadius.circular(18),
          border: Border.all(color: AppColors.border, width: 1),
        ...
        child: ClipRRect(
          ...
          child: _SettingsRow(
            ...
            title: '$_offlinePending Absensi Menunggu Sync',
            subtitle: _syncing ? 'Sedang menyinkronkan...' : 'Tap untuk sinkronkan sekarang',
...
// ─────────────────────────────────────────────────────────────────────────────
// NEW INFO ROW — Desain baru: ikon kotak kiri, label abu di atas, nilai bold
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// SETTINGS SECTION & ROW — Grouped styling for premium visual experience
// ─────────────────────────────────────────────────────────────────────────────
